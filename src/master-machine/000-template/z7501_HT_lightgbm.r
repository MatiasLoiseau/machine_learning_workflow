...
PARAM  <- list()
PARAM$experimento  <- "HT7501"

PARAM$exp_input  <- "TS7401"

PARAM$lgb_crossvalidation_folds  <- 5  #En caso que se haga cross validation, se usa esta cantidad de folds

PARAM$lgb_semilla  <- 374063   #cambiar por su propia semilla
...
exp_log  <- function( reg, arch=NA, folder="./exp/master-machine/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./exp/master-machine/", PARAM$exp_input, "/dataset_training.csv.gz" )
